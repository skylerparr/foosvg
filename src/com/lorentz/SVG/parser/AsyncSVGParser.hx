...
class AsyncSVGParser extends EventDispatcher
{
    ...
    private function visit(visitDefinition : VisitDefinition) : Array<Dynamic>
    {
        ...
        if (Std.is(obj, SVGElement))
        {
            ...
            element.metadata = elt.nodes.svg::metadata.get(0);
    ...
    private function parseFilterColorMatrix(node : FastXML) : SVGColorMatrix
    {
        ...
        var valuesString : String = ((Lambda.has(node, "@values"))) ? node.att.values : "";
        var values : Array<Dynamic> = [];
        for (v/* AS3HX WARNING could not determine type for var: v exp: ECall(EField(EIdent(SVGParserCommon),splitNumericArgs),[EIdent(valuesString)]) type: null */ in SVGParserCommon.splitNumericArgs(valuesString))
        {
